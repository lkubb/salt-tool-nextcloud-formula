...
We will see a quick setup to configure the ``tool_nextcloud`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_nextcloud`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_nextcloud`` formulas under ``/srv/salt/tool_nextcloud/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_nextcloud/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_nextcloud/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_nextcloud/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_nextcloud/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_nextcloud`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_nextcloud`` formulas under ``/srv/salt/tool_nextcloud/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_nextcloud/parameters/roles

We will define 2 roles:

- ``tool_nextcloud/server``
- ``tool_nextcloud/client``

We create a configuration for the role ``tool_nextcloud/server`` in ``/srv/salt/tool_nextcloud/parameters/roles/tool_nextcloud/server.yaml``:
...
We create another configuration for the role ``tool_nextcloud/client`` in ``/srv/salt/tool_nextcloud/parameters/roles/tool_nextcloud/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_nextcloud/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_nextcloud/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_nextcloud/parameters/dns:domain/  '/srv/salt/tool_nextcloud/parameters/dns!domain/'
